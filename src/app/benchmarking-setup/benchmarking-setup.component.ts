...
export class BenchmarkingSetupComponent implements OnInit, OnDestroy {
  ...
  handleMissingRates(pa: IBMPracticeArea, collections: Array<any>): void {
    if (collections && collections.length > 0) {
      ...
      const found = collections.find(e =>  e.bh_lawfirm_id === this.benchmark.firm_id && e.law_firm_id === this.benchmark.firm_id);
      if (found && (found.status === 'BIDDING' || found.status === 'INVITED')) {
